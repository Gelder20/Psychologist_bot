...
Когда клиент запускает бота, начинается опрос из различных вопросов с вариантами ответа. После бот оценивает психо-эмоциональное состояние и сообщает результат клиенту. В конце идёт анкета, где просят оценить своё эмоциональное состояние от 1 до 10. Далее клиенту предлагают начать диалог, и начинается процесс общения с ботом. Во время общения можно закончить диалог или вызвать специалиста службы поддержки. Если закончить диалог, будет повторно вызвана анкета и после бот выведет результат его работы (стало ли хуже, лучше или ничего не изменилось). Если обратиться в поддержку, начнётся процесс общения специалиста с клиентом через бота. Также, если бот выявит плохой эмоциональный окрас сообщения во время общения клиента с ботом, то специалист будет вызван автоматически.
