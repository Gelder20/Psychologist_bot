...
Обратиться в службу поддержки можно на любом этапе взаимодействия с ботом. После обращения, когда свободный специалист его примет, клиент будет общаться со специалистом службы поддежки через бота. Также, если бот выявит плохой эмоциональный окрас сообщения во время общения клиента с ботом, то специалист будет вызван автоматически.

**От лица специалиста службы поддержки:**

Когда специалист запускает бота, ему отправляется уведомление об ожидании вызова, когда вызов появляется, специалисту приходит уведомление.
